    ...
    public void runDevicesUntilTargetWithIncrementalWrite() {
        ...
        try {
            ...
            while (currentMaliciousCount < targetMaliciousMessages && iteration < maxIterations) {
                iteration++;
                // Stamped onto every row of this batch. Batches are contiguous
                // segments of one continuous stream, so they are traceability
                // markers, not independent units - group on run_id/trace_id.
                RunContext.batchIndex = iteration;
            ...
            RunContext.writeManifest(filename);
            Logger.getLogger("BalancedSamambaiaScenario").info(RunContext.summary());
