...
public class SetupIED extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (request.getParameter("iedName") == null) { // handles the JSON body (e.g., from API)
            ...
            iedName = setupIED.iedName;
            gocbRef = setupIED.gocbRef;
            datSet = setupIED.datSet;
            minTime = setupIED.minTime;
            maxTime = setupIED.maxTime;
            timestamp = setupIED.timestamp;
            stNum = setupIED.stNum;
            sqNum = setupIED.sqNum;
